...
async function initDatabase() {
  if (DB_TYPE === 'sqlite') {
    ...
    // Alpine 시스템 CA 번들을 명시적으로 전달해야 mysql2 v3.x에서
    // CLIENT_SSL 플래그가 제대로 설정됩니다 (RDS require_secure_transport=ON 대응)
    const sslOptions = { rejectUnauthorized: false };
    try {
      sslOptions.ca = require('fs').readFileSync('/etc/ssl/certs/ca-certificates.crt');
    } catch (_) {
      // 로컬 개발 환경 등 파일이 없으면 무시
    }
